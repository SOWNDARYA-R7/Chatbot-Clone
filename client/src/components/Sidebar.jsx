  ...
  return (
    ...
    transition-all duration-500 max-md:absolute left-0 z-1'>
      ...
      dark:border-white/20 rounded-md">
        ...
        className="text-xs placeholder:text-grey-400 outline-none"/>
      </div>
      {/* Recent Chats */}
      ...
      <div className="flex-1 overflow-y-scroll mt-3 text-sm space-y-3">
        {
          ...
          includes(search.toLowerCase())).map((chat) => (
            ...
            border-grey-300 dark-border-[#80609F]/15 rounded-md cursor-pointer
            flex justify-between group">
              <div>
                ...
                <p className="text-xs text-grey-500 dark:text-[#B1A6C0]">{chat.updatedAt}</p>
              </div>
